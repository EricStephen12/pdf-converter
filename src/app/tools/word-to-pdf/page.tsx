...
export default function WordToPDF() {
  ...
  const handleConvert = async () => {
    ...
    try {
      // For Word to PDF, we use print-to-PDF approach via iframe
      const arrayBuffer = await file.arrayBuffer();
      const blob = new Blob([arrayBuffer], { type: file.type });
      const url = URL.createObjectURL(blob);
      
      // Create a hidden iframe to trigger print dialog
  ...
  return (
    <ToolLayout
      title="Word to PDF"
      description="Convert your Word documents (.doc, .docx) to PDF format. Uses your browser's print function for best compatibility."
      icon="📝"
      color="blue"
    >
      {!file ? (
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-indigo-500 rounded-xl flex items-center justify-center text-xl">
                📝
              </div>
              <div>
